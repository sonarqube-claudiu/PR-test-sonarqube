...
const IssueHistoryTimeline = () => {
  const intl = useIntl();
  const issueToBeUpdated = useSelector(state => state.issues.issueToBeUpdated);
  const journals = useSelector(
    state => state.issueJournals.journals[issueToBeUpdated?.id]
  );
  const loading = useSelector(state => state.issueJournals.loading);
  return (
    <div className="timeline-vertical py-0">
      ...
      {!loading && (!journals || journals?.length === 0) && (
        <div className="d-flex flex-row justify-content-center align-items-center">
          <FormattedMessage id="history.withoutHistory" />
        </div>
      )}
    </div>
  );
